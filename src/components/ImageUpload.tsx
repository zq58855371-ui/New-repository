    ...
    <>
      ...
      >
        {currentImage ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center bg-white/5 border-2 border-dashed border-white/20 hover:border-primary/50 transition-colors">
            ...
            >
              {previewUrl ? (
                ...
                <>
                  ...
                  <p className="text-muted-foreground">点击选择图片或拖拽到此处</p>
                </>
              )}
              <input
                ref={fileInputRef}
...
interface PhotoGalleryProps {
  ...
  onUpdate: (id: string, updates: { caption?: string }) => void;
  isEditMode: boolean;
}

export function PhotoGallery({ photos, onAdd, onDelete, onUpdate, isEditMode }: PhotoGalleryProps) {
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [previewUrl, setPreviewUrl] = useState<string>('');
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
              <div className="absolute bottom-0 left-0 right-0 p-3">
                {isEditMode ? (
                  ...
                  <p className="text-white text-sm truncate">{photo.caption || '无描述'}</p>
                )}
